...
public class ServiciosManager {
    ...
    public async Task<IEnumerable<ServicioListViewModel>> ObtenerTodosAsync() {
        List<Servicio> servicios = await _dbContext.Servicios
            .AsNoTracking()
            .Include(s => s.Area)
        ...
        return servicios.Select(s => new ServicioListViewModel(servicio: s));
    }
    ...
    public async Task<ServicioDetalleViewModel> ObtenerDetallePorIdAsync(Guid id) {
        Servicio servicio = await _dbContext.Servicios
            .Include(s => s.Area)
            .Include(s => s.Modalidad)
            .AsNoTracking()
            .FirstOrDefaultAsync(s => s.Id == id);
        ...
        List<Proyecto> proyectos = await _dbContext.Sesiones
        ...
        List<Proyecto> proyectosUnicos = proyectos
            .GroupBy(p => p.Id)
            .Select(g => g.First())
            .OrderBy(p => p.Nombre)
            .ToList();

        ServicioDetalleViewModel viewModel = new() {
            Id = servicio.Id,
    ...
    public async Task<(bool Success, ServicioListViewModel Data, string Error)> CrearAsync(
        ...
        try {
            Area area = await _dbContext.Areas.FindAsync(Guid.Parse(input: viewModel.IdArea));
            if (area == null || area.IsDeleted) {
                return (false, null, "El área seleccionada no existe o fue eliminada");
            }

            Modalidad modalidad = await _dbContext.Modalidades.FindAsync(Guid.Parse(input: viewModel.IdModalidad));
            if (modalidad == null || modalidad.IsDeleted) {
                return (false, null, "La modalidad seleccionada no existe o fue eliminada");
            }

            Servicio servicio = viewModel.ToEntity();
            servicio.RegristrarCreacion(creadoPor: usuarioActual, creadoEl: DateTime.UtcNow);

            await _dbContext.Servicios.AddAsync(entity: servicio);
            int changes = await _dbContext.SaveChangesAsync();

            if (changes > 0) {
                servicio = await ObtenerPorIdAsync(id: servicio.Id);
                ServicioListViewModel result = new(servicio: servicio);
            ...
            _logger.LogError(exception: ex, "Error al crear servicio");
    ...
    public async Task<(bool Success, ServicioListViewModel Data, string Error)> ActualizarAsync(
        ...
        string usuarioActual) {
        try {
            Servicio servicio = await _dbContext.Servicios.FindAsync(Guid.Parse(input: viewModel.Id));
            ...
            Servicio updatedServicio = viewModel.ToEntity();
            servicio.Actualizar(servicio: updatedServicio, actualizadoPor: usuarioActual);

            _dbContext.Servicios.Update(entity: servicio);
            int changes = await _dbContext.SaveChangesAsync();
            ...
            _logger.LogError(exception: ex, "Error al actualizar servicio con ID: {Id}", viewModel.Id);
    ...
    public async Task<(bool Success, string Error)> EliminarAsync(Guid id, string usuarioActual) {
        try {
            ...
            servicio.Eliminar(eliminadoPor: usuarioActual);
            _dbContext.Servicios.Update(entity: servicio);
            int changes = await _dbContext.SaveChangesAsync();
            ...
            _logger.LogError(exception: ex, "Error al eliminar servicio con ID: {Id}", id);
    ...
    public async Task<bool> ExisteNombreAsync(string nombre, Guid excludeId = default) {
        IQueryable<Servicio> query = _dbContext.Servicios
