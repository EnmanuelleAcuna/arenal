...
public class AgregarRolViewModel {
    ...
    [StringLength(maximumLength: 250, ErrorMessage = "La descripción no debe exceder los 250 caracteres.")]
    public string Descripcion { get; set; }

    public ApplicationRole ToApplicationRole() {
        return new ApplicationRole(Guid.NewGuid().ToString(), nombre: Nombre, descripcion: Descripcion);
    }
}
...
public class EditarRolViewModel {
    ...
    [Display(Name = "Descripción")]
    [Required(ErrorMessage = "La descripción es requerida.")]
    [StringLength(maximumLength: 250, ErrorMessage = "La descripción no puede exceder los 250 caracteres.")]
    public string Descripcion { get; set; }
}